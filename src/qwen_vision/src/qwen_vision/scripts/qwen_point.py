...
class ObjectLocatorNode:
    def __init__(self):
        ...
        # ROS参数配置
        self.model_name = rospy.get_param('~model', 'qwen3.6-plus')
        self.default_prompt = rospy.get_param('~prompt', '请只定位目标物体的位置（只返回bbox）')
        ...
        self.api_key = rospy.get_param('~api_key', "sk-87234c4787964295b2bc8687b1814656")
        self.base_url = rospy.get_param('~base_url', "https://dashscope.aliyuncs.com/compatible-mode/v1")
        if not self.api_key:
            rospy.logwarn("DASHSCOPE api_key is empty! Set rosparam ~api_key or env DASHSCOPE_API_KEY.")

        # ===== [MOD] 同步服务等待超时（秒）=====
        self.locate_timeout = float(rospy.get_param('~locate_timeout', 10.0))

        # ===== [MOD] 深度有效范围（米）=====
        self.min_depth_m = float(rospy.get_param('~min_depth_m', 0.10))
        self.max_depth_m = float(rospy.get_param('~max_depth_m', 3.00))
        ...
        self.target_name = rospy.get_param('~target', '')  # 例如 "water bottle"
        ...
        rospy.loginfo("Service: /locate_object (async), /locate_object_sync (sync)")
        rospy.loginfo("Topic input: /voice_text, /target_object")
        rospy.loginfo("Topic output: /object_position, /object_name, /image_description")
    ...
    def init_ros_interfaces(self):
        ...
        # 相机内参订阅
        self.info_sub = rospy.Subscriber('/camera/aligned_depth_to_color/camera_info', CameraInfo, self.camera_info_callback, queue_size=1)

        # 语音文本 接收外部语音识别模块发来的文字信息
        self.voice_sub = rospy.Subscriber('/voice_text', String, self.voice_text_callback, queue_size=1)

        # ===== [MOD] 上位机可直接发布目标物名 =====
        self.target_sub = rospy.Subscriber('/target_object', String, self.target_object_callback, queue_size=1)

        # 结果发布（latch=True：上层随时能拿到最新一次结果）
        self.visualization_pub = rospy.Publisher('/visualization', Image, queue_size=1)
        self.position_pub = rospy.Publisher('/object_position', PoseStamped, queue_size=1, latch=True)
        self.object_name_pub = rospy.Publisher('/object_name', String, queue_size=1, latch=True)
        self.description_pub = rospy.Publisher('/image_description', String, queue_size=1, latch=True)

        # 提示词更新（如果你还需要外部覆盖 prompt，可以保留）
        self.prompt_sub = rospy.Subscriber('/prompt_update', String, self.prompt_update_callback, queue_size=1)

        # 定位服务（异步）
        self.trigger_service = rospy.Service('/locate_object', Trigger, self.handle_locate_request)
        # ===== [MOD] 定位服务（同步，推荐 robot_aiui.cpp 用这个）=====
        self.sync_service = rospy.Service('/locate_object_sync', Trigger, self.handle_locate_request_sync)

        # 交互式定位服务（保留）
        self.interactive_service = rospy.Service('/interactive_locate', Trigger, self.handle_interactive_request)
    ...
    def target_object_callback(self, msg: String):
        ...
        rospy.loginfo("Target object updated: %s", self.target_name)
    ...
    def voice_text_callback(self, msg):
        ...
        if not target:
            rospy.logwarn("Voice text received but target not recognized: %s", text)
            return
        ...
        rospy.loginfo("Target from voice: %s | Prompt: %s", self.target_name, self.default_prompt)
    ...
    def prompt_update_callback(self, msg):
        ...
        try:
            ...
            rospy.loginfo("Prompt updated (plain text): %s", self.default_prompt)
    ...
    def fail_publish(self, log_detail: str, msg_cn: str = "目标检测失败"):
        ...
        """
        rospy.logwarn(log_detail)
        self.description_pub.publish(String(msg_cn))
    ...
    def handle_locate_request(self, req):
        ...
        with self.state_lock:
            if self.state != NodeState.STANDBY:
                return TriggerResponse(success=False, message="Previous request is still processing")
        ...
        with self.state_lock:
            if self.color_frame is None or self.depth_frame is None:
                self.state = NodeState.STANDBY
                return TriggerResponse(success=False, message="Camera data not available")
        ...
        return TriggerResponse(success=True, message="Object localization started (async)")

    def handle_locate_request_sync(self, req):
        """同步定位：阻塞等待结果，返回 JSON（成功）或失败原因（失败）"""
        resp = self.handle_locate_request(req)
        if not resp.success:
            return resp

        if not self.done_event.wait(self.locate_timeout):
            rospy.logwarn("Timeout: no result within %.1fs", self.locate_timeout)
            return TriggerResponse(False, "目标检测失败")

        with self.last_result_lock:
            if not self.last_success or self.last_pose is None:
                return TriggerResponse(False, self.last_message or "Locate failed")
    ...
    def handle_interactive_request(self, req):
        rospy.loginfo("Interactive mode: Locating objects with current target/prompt")
    ...
    def detect_objects_with_vision_model(self, image_path, prompt, target_name: str):
        ...
        try:
            ...
            if not obj_data or 'objects' not in obj_data:
                rospy.logerr("Failed to parse JSON from model. Raw: %s", content)
                return None

            return obj_data

        except requests.exceptions.RequestException as e:
            rospy.logerr("Vision model API request failed: %s", str(e))
            return None
        except Exception as e:
            rospy.logerr("detect_objects_with_vision_model error: %s", str(e))
            return None
    ...
    def process_results_publish_one(self, selected_obj, timestamp):
        ...
        try:
            ...
            transform = self.tf_buffer.lookup_transform("map", source_frame, tf_time, rospy.Duration(1.0))
            ...
            rospy.logwarn("TF to map failed, keep camera frame. err=%s", str(e))

        # 发布
        self.position_pub.publish(pose_out)
        self.object_name_pub.publish(String(selected_obj.get("name", "unknown")))
    ...
    def publish_visualization(self, vis_img, timestamp):
        try:
            ...
            rospy.logwarn("Failed to publish visualization: %s", str(e))
    ...
    def process_detection(self, color_img, depth_img, camera_info, timestamp, prompt, target_name):
        ...
        try:
            ...
            self.description_pub.publish(String(desc))
