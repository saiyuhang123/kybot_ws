    ...
    return LaunchDescription([
        Node(
            ...
            parameters=[
                {"frame_name": "laser_he",
                 "topic_name": "scan_he",
                 "service_name": "HinsheSrv",

                 "laser_type": 1,                   #  1:he  2:fe 3: se  4: le  # 设置连接传感器系列；

                 "laser_ip": "192.168.1.88",		# 传感器ip地址		
                 "port": 8080,				        # 传感器端口（固定为8080）
                 "use_udp": False,                   # True：UDP链接方式；False：TCP链接方式；
                 "synctype": False,                 # True：读取NTP格式报文（传感器版本号大于2.3.0）；False：获取标准报文；                 

                 "block_enable": False,             # True：获取障碍物检测结果；False：不获取障碍物检测；   
                 "change_param": False,              # 是否改变雷达运行参数（False不改变：True改变）;
                 "angle_increment": "0.100",        # 设置运行分辨率（0.025；0.050；0.100；0.250；0.500);
                 "spin_frequency_Hz": 15,           # 设置扫描频率HE-3091F系列（15:15hz；30；30hz）HE-3051，HE-3081系列（15:10hz；30；20hz）;
                 "noise_filter_level": 1,           # 噪声过滤等级（0：关闭;1:简单；2：中等；3：严格；） ;

                 "start_angle": -160.0,                # 设置输出角度最小值-180.0--180.0，过滤角度值强制等于1024；
                 "end_angle": 160.0,                # 设置输出角度最大值-180.0--180.0，过滤角度值强制等于1024；
