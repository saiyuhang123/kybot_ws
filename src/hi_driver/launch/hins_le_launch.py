    ...
    return LaunchDescription([
        Node(
            ...
            parameters=[
                {"frame_name": "laser_le",
                 "topic_name": "scan_le",
                 "service_name": "HinsleSrv",

                 "laser_type": 4,                   # 设置连接传感器系列； 1:he   2:fe 3: se  4: le

                 "laser_ip": "192.168.1.88",		# 传感器ip地址		
                 "port": 8080,				        # 传感器端口（固定为8080）
                 "use_udp": False,                   # True：UDP链接方式；False：TCP链接方式；                               
                 "synctype": False,                 # True：读取NTP格式报文（LE-50XXFA系列版本号大于1.3.0支持）；False：获取标准报文；

                 "block_enable": False,             # True：获取障碍物检测结果；False：不获取障碍物检测；   
                 "change_param": False,              # 是否改变雷达运行参数（False：不改变：True：改变）;
                 "spin_frequency_Hz": 30,           # 设置扫描频率（10:10hz；15:15hz；20:20hz；20：25hz；:30：30hz）;
                 "measure_frequency_kHz": "200",    # 设置测量频率（固定：200:200k）;
                 "sampling_size_per_position": 1,   # 单点采样次数（1-50）;
                 "noise_filter_level": 0,           # 噪声过滤等级（0：关闭;1:简单；2：中等；3：严格；） ;

                 "start_angle": 0.0,                # 设置输出角度最小值-180.0--180.0，过滤角度值强制等于1024；
                 "end_angle": 0.0,                # 设置输出角度最大值-180.0--180.0，过滤角度值强制等于1024；
