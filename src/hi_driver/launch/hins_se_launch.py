    ...
    return LaunchDescription([
        Node(
            ...
            parameters=[
                {"frame_name": "laser_se",
                 "topic_name": "scan_se",
                 "service_name": "HinsseSrv",                 

                 "laser_type": 3,                   # 设置连接传感器系列； 1:he   2:fe 3: se  4: le
                 "laser_ip": "192.168.1.88",	    # 传感器ip地址		
                 "port": 8080,				        # 传感器端口（固定为8080）
                 "use_udp": False,                  # True：UDP链接方式；False：TCP链接方式；                  

                 "block_enable": True,              # True：获取障碍物检测结果；False：不获取障碍物检测；   

                 "start_angle": -135.0,                # 设置输出角度最小值-180.0--180.0，过滤角度值强制等于1024；
                 "end_angle": 135.0,                # 设置输出角度最大值-180.0--180.0，过滤角度值强制等于1024；
