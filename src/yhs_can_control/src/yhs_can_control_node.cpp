...
namespace yhs
{
	...
	CanControl::CanControl(rclcpp::Node::SharedPtr node)
			: node_(node), if_name_("can1"), can_socket_(-1), wheel_base_(0.6), odom_scale_factor_(1.0)
	{

		READ_PARAM(std::string, "can_name", (if_name_), "can1");
		READ_PARAM(double, "wheel_base", (wheel_base_), 0.6);
		READ_PARAM(double, "odom_scale_factor", (odom_scale_factor_), 1.0);
...
whether the can line is connected correctly, and whether the chassis is powered on.");
	...
	void CanControl::publish_odom(const double velocity, const double steering)
	{
		...
		static rclcpp::Time last_odom_stamp_ = node_->now();

		rclcpp::Time current_time = node_->now();

		// 保证 header.stamp 严格单调递增，防止系统时间跳变导致下游
		// lidar_localization 报 "odom time interval is negative"
		if (current_time <= last_odom_stamp_) {
			current_time = rclcpp::Time(last_odom_stamp_.nanoseconds() + 1);
		}
		...
		double linear_vel = velocity * odom_scale_factor_;
		...
		// // 5. 平移 — 直接用已有的 x_, y_, 0.0
		// tf_msg.transform.translation.x = x_;
		// tf_msg.transform.translation.y = y_;
		// tf_msg.transform.translation.z = 0.0;	
		// // 6. 旋转 — quat 已经在上面第 452-457 行算好了，直接复用
