...
static void client_gpgga_callback(const msg_interfaces::msg::String::ConstPtr msg)
{
    ...
    const string &s = msg->sentence;

    // 兼容所有 talker ID 的 GGA: $GPGGA / $GNGGA / $GLGGA / $GAGGA / $GBGGA / $GQGGA ...
    bool is_gga = (s.size() >= 6) && (s[0] == '$') && (s.compare(3, 3, "GGA") == 0);
    bool fid_match = (frame_id.compare(msg->header.frame_id) == 0);

    if (is_gga && fid_match)
    {
        swas_sdk_upload_gga(s.c_str(), s.size());
    }
}
