...
void initSkills(SkillRegistry &reg) {
    ...
    reg.registerSkill({
        ...
        [](const JsonValue &/*params*/) -> SkillResult {
            ...
            // 阻塞等待视觉识别结果（内部用 rclcpp::spin_some() 轮询 /image_description 话题）
...
void visionDescriptionCallback(const std_msgs::msg::String::SharedPtr msg)
{
  RCLCPP_INFO(rclcpp::get_logger("agent_bridge"), "I heard: [%s]", msg->data.c_str());
}
